...
int 	gsocket;   		// ñapa para discriminar claro/seguro
...
#ifdef COMMUNICATION_M95
	if (sock == 1){
		if (HAL_UART_Transmit(&huart6,buffer,lengthBuffer,3000)==HAL_OK) return 1;
		else return -1;
	}
	else {
		// Call the TLS Flow for sending command
		CmdProps *CommandList = QSendFlow(buffer, lengthBuffer);
		int	i = ATCommandFlow(CommandList, &huart6, WDT_ENABLED, &hiwdg, &timeoutGPRS, 0);
...
int rcvstate = 0 ; // 0 -> buff empty for sure (0) or maybe not(1)
static char	*urc = "\r\n+QSSLURC: \"recv\",0,1\r\n";
static char *recvfmt =  "AT+QSSLRECV=0,1,%d\r";
...
int transport_getdata(unsigned char* buffer, int count) {
	...
	//		HAL_NVIC_EnableIRQ(USART6_IRQn);
		...
		char	work[128];					// safe place where GET type Command QSSLRECV reply work with
			...
			{
		CmdProps ReadCmd =  {ATGET,	"",		{readcmd},	{NULL, work, getRecvPayload},	{200, 0}, 	1};
		int num = executeCommand (&ReadCmd, &huart6);
		if (num) {
			for (i = 0; i < num; i++){
				buffer[i] = work[i];
			}
...
// Handler for QSSLRECV command, intended to retrieve just the second part (payload) and move to begin
int					getRecvPayload(const char *reply){
	int	ip1, ip2, ip3, ip4, port;
	int nbytes;
	if (!strncmp(reply, "\r\nOK\r\n", 6)) {
		reply = reply + 6;
	}
	if (!strncmp(reply, urc, strlen(urc))) {
		reply = reply + strlen(urc) ;
	}
...
#include "MQTTAPI.H"
			extern int hmqtt;
			if (strcmp(reply, urc)) {
				tprintf (hmqtt, "UNEXPECTED #%s#", reply);
			}
#endif
...
M95Status Modem_Init() {
	...
	M95Status rc = M95_Initialize( &huart6,WDT_ENABLED, &hiwdg, Emerg_GPIO_Port, Emerg_Pin,
			Pwrkey_GPIO_Port, Pwrkey_Pin,
...
//		return -1;
	...
	if (!strcmp(CommandList[i].id,"END")){
		...
		if (0) {
			...
			int num = executeCommand (&DeactCmd, &huart6);

		}
		if (1) {
			int j = DisconnectPlus (WDT_ENABLED,&hiwdg, &huart6, &timeoutGPRS, 1000, &rebootSystem);
		}
		if (0) {
